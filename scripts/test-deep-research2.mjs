...
if (!process.env.LLM_API_KEY) {
  console.log("[SKIP] test-deep-research2 requires LLM_API_KEY in env");
  process.exit(0);
}
...
const decomp = await runAgent(
  `Break down this question into 1-2 specific sub-questions, each starting with "- ":\nQuestion: "${question}"`,
  "auto"
);
console.log(`  took ${Date.now() - t0}ms`);
const subs = decomp.answer
  .split("\n")
